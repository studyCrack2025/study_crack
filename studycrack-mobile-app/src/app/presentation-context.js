...
export function buildAppPresentations({ state, derived, liveSeconds }) {
  ...
  return { studyOverview, aquariumPresentation, myPresentation, targetPolicy: buildTargetPolicy(state), analysisPresentation: buildAnalysisSnapshot(state), streakPresentation: buildStreakPresentation(state), aquariumShareText: aquariumShareText(aquariumPresentation) };
}
