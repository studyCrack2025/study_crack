...
function MbtiIntro() {
  return (
    <div className="mbti-survey-intro">
      ...
      <button type="button" className="btn btn-primary mbti-survey-start" data-action="startMbti">검사 시작 (약 2분)</button>
  ...
  return (
    <div className="mbti-survey-q">
      <div className="mbti-survey-head">
        ...
        <button type="button" className="qna-modal-close" data-action="closeMbtiModal" aria-label="닫기">✕</button>
      </div>
      <div className="mbti-survey-progress"><div className="mbti-survey-bar"><i style={{ width: `${percent}%` }} /></div><span>{step + 1} / {TOTAL}</span></div>
      <p className="mbti-survey-qtext">{item.q}</p>
      <div className="mbti-survey-choices">
        <button type="button" className={`mbti-survey-choice ${picked === 1 ? 'selected' : ''}`} data-action="answerMbti" data-mbti-step={step} data-mbti-choice="1"><span className="mbti-survey-choice-mark">A</span><span>{item.a}</span></button>
        <button type="button" className={`mbti-survey-choice ${picked === 2 ? 'selected' : ''}`} data-action="answerMbti" data-mbti-step={step} data-mbti-choice="2"><span className="mbti-survey-choice-mark">B</span><span>{item.b}</span></button>
      </div>
...
function MbtiResult({ code }) {
  ...
  return (
    <div className="mbti-survey-result">
      ...
      <div className="mbti-result-code">{profile.code}</div>
      <div className="mbti-result-name">{profile.name}</div>
...
export function MbtiModal({ mbtiAnswers = [], mbtiModalOpen = false, mbtiResult = '', mbtiStep = 'intro' }) {
  ...
  return <Modal panelClass="mbti-survey-modal" dismissAction="closeMbtiModal">{content}</Modal>;
}
