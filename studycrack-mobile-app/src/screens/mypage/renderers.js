...
// 학습 유형(MBTI) 카드. 저장된 코드가 있으면 실제 프로필을, 없으면 검사 유도를 보여준다.
...
export function renderSettingsMainScreen(ctx) {
  ...
  return layout(appbar('설정', true) + `<div class="sc-secondary-page settings-page">${renderSecondaryIntro({ eyebrow: 'SETTINGS', title: '설정', description: '계정과 약관, 로그인 상태를 관리합니다.' })}<div class="sc-secondary-list settings-list"><button class="sc-secondary-row" data-action="goto" data-target="accountInfo"><span class="sc-secondary-row-main"><b>계정 정보</b><p>프로필과 로그인 계정 관리</p></span><span>${icon('chevron', false)}</span></button><button class="sc-secondary-row" data-action="goto" data-target="settingsTermsPicker"><span class="sc-secondary-row-main"><b>약관 보기</b><p>이용약관과 개인정보 처리방침</p></span><span>${icon('chevron', false)}</span></button><button class="sc-secondary-row" data-action="openLogoutModal"><span class="sc-secondary-row-main"><b>로그아웃</b><p>현재 기기에서 로그인 종료</p></span><span>${icon('chevron', false)}</span></button></div></div>`, false, `${renderLogoutModal(logoutModalOpen)}${renderTermsModal(openTermsType, termsContent)}`);
}
...
export function renderSettingsTermsPickerScreen(ctx) {
  ...
  return layout(appbar('약관 보기', true) + `<div class="sc-secondary-page terms-picker-page">${renderSecondaryIntro({ eyebrow: 'LEGAL', title: '약관 및 정책', description: '서비스 이용과 개인정보 처리 기준을 확인할 수 있어요.' })}<div class="sc-secondary-list settings-list">${[['standard','표준 이용약관'],['privacy','개인정보 처리방침'],['service','서비스 이용약관'],['refund','환불규정'],['marketing','마케팅 수신 정보 동의']].map(([key,label]) => `<button type="button" class="sc-secondary-row" data-action="openTermsModal" data-terms-type="${key}"><span class="sc-secondary-row-main"><b>${label}</b><p>전문 보기</p></span><span>${icon('chevron', false)}</span></button>`).join('')}</div></div>`, false, renderTermsModal(openTermsType, termsContent));
}
...
export function renderAccountInfoScreen(ctx) {
  ...
  return layout(appbar('계정 정보', true) + `<div class="sc-secondary-page account-info-page">
    ${renderSecondaryIntro({ eyebrow: 'ACCOUNT', title: escapeHtml(displayName(user)), description: `${escapeHtml(displayEmail(user))} · ${escapeHtml(displayPlan(selectedPlan))}`, aside: '<span class="sc-chip">계정 관리</span>' })}
    <section class="sc-secondary-section mobile-account-card">
      <div class="sc-secondary-section-head account-section-head"><div><h3>프로필</h3><p>서비스와 리포트에 표시되는 정보입니다.</p></div><span>기본 정보</span></div>
    ...
    <section class="sc-secondary-section mobile-account-card">
      <div class="sc-secondary-section-head account-section-head"><div><h3>로그인 및 연락처</h3><p>인증 정보와 마케팅 수신 여부를 관리합니다.</p></div><span>${escapeHtml(providerLabel(authProvider) || 'Local')}</span></div>
    ...
    <section class="sc-secondary-section mobile-account-card">
      <div class="sc-secondary-section-head account-section-head"><div><h3>소셜 계정 연동</h3><p>Google과 Naver 계정을 연결하거나 해제합니다.</p></div><span>2개 제공</span></div>
      <div class="mobile-social-list">${renderSocialAccountRows(user)}</div>
    </section>
    <section class="sc-secondary-section mobile-account-card danger-zone">
      <div class="sc-secondary-section-head"><div><h3>구독 및 탈퇴</h3><p>현재 플랜을 확인하고 계정 탈퇴를 요청합니다.</p></div></div>
...
export function renderPrivacyPolicyScreen({ appbar, layout, termsContent = TERMS_CONTENT }) {
  const content = termsContent.privacy || TERMS_CONTENT.privacy;
  return layout(appbar(content.title, true) + `<div class="sc-reading-page">${renderSecondaryIntro({ eyebrow: 'PRIVACY', title: content.title, description: '개인정보의 수집·이용·보관 기준을 안내합니다.' })}<div class="sc-reading-content">${escapeHtml(content.body)}</div></div>`, false);
}

export function renderTermsScreen({ appbar, layout, termsContent = TERMS_CONTENT }) {
  const content = termsContent.service || TERMS_CONTENT.service;
  return layout(appbar(content.title, true) + `<div class="sc-reading-page">${renderSecondaryIntro({ eyebrow: 'TERMS', title: content.title, description: '서비스 이용 조건과 책임 범위를 확인해주세요.' })}<div class="sc-reading-content">${escapeHtml(content.body)}</div></div>`, false);
}
