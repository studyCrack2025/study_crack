...
export function FishDexPanel({ selection, setSelection, catalog = [], error = '', inventory = [], profile, status = 'idle' }) {
  ...
  const setCategory = (value) => setSelection(current => ({ ...current, category: value, page: 1 }));
  ...
  const visibleCatalog = RARITY_ORDER.flatMap(rarity => categoryCatalog.filter(fish => fish.rarity === rarity && (selectedRarity === 'all' || selectedRarity === rarity) && (filter === 'all' || (filter === 'owned' ? fish.owned || ownedSpecies.has(fish.speciesId) : !(fish.owned || ownedSpecies.has(fish.speciesId))))));
  const pages = Math.ceil(visibleCatalog.length / 12);
  const page = Math.min(Math.max(1, requestedPage), Math.max(1, pages));
  const pageCatalog = visibleCatalog.slice((page - 1) * 12, page * 12);
  const changePage = value => {
    moveFocus.current = true;
    setSelection(current => ({ ...current, page: value }));
  };
  useLayoutEffect(() => {
