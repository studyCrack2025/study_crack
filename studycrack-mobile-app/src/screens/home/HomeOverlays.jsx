...
export function StudySubjectSheet({
  ...
  return (
    <Sheet open={studySubjectSheetOpen} panelClass="study-subject-sheet" dismissAction="closeStudySubjectSheet">
      <h3>어떤 과목을 공부할까요?</h3>
      {!studySubjectSheetOnlyPlanned ? <div className="study-subject-grid">{DEFAULT_STUDY_SUBJECTS.map((subject) => <button className="planner-pill" data-action="selectStudySubject" data-study-subject={subject} key={subject}>{subject}</button>)}<button className="planner-pill" data-action="selectStudySubjectCustom">기타 직접 입력</button><button className="planner-pill" data-action="selectSelfStudy">자율공부</button></div> : null}
      {plannedScheduleOptions.length ? <><p className="sub" style={{ margin: '8px 0 6px' }}>오늘 플래너 일정</p><div className="study-subject-grid">{plannedScheduleOptions.map((row) => <button className="planner-pill" data-action="selectStudySubject" data-study-subject={row.subject} data-study-item-id={row.id} key={row.id || row.label}>{row.label}</button>)}</div></> : <p className="sub" style={{ marginTop: 8 }}>오늘 플래너 일정이 없습니다.</p>}
    </Sheet>
  );
}
