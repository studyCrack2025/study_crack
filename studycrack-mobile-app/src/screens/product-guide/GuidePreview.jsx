...
export function GuidePreview({ step, profile, aquarium, catalog, tasks = [] }) {
  ...
  if (step === 3) return <div className="product-guide-scene"><AquariumScene backgroundKey={aquarium?.backgroundKey} variant="guide" slots={aquarium?.slots} catalog={catalog} /><span>{aquarium?.activeCount ? '확정 공부 5시간마다 뽑기권을 받아요.' : '물고기를 수조에 배치하면 여기에 보여요.'}</span></div>;
