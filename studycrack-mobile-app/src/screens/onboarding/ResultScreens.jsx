...
export function Ob5Screen(ctx) {
  const {
    ...
    analysisPresentation = {}, crackySrc = CRACKY_SRC,
    gaugePassPct = 40, gaugeSafePct = 60, scoreTierClass = defaultScoreTierClass
  } = ctx;
  const hasScore = analysisPresentation.ready && typeof analysisPresentation.score === 'number' && Number.isFinite(analysisPresentation.score);
  const gaugeCurrent = hasScore ? analysisPresentation.score : 0;
  const gaugeCurrentPct = Math.min(100, gaugeCurrent / GAUGE_MAX * 100);
  const targetScore = gaugeCurrent;
  const targetPct = Math.min(100, targetScore / GAUGE_MAX * 100);
  const isLoading = analysisApiStatus === 'loading';
  const actualTargets = analysisPresentation.ready ? (analysisPresentation.comparison || []).filter((item) => item?.major && typeof item.score === 'number' && Number.isFinite(item.score)) : [];
  return <OnboardingScreenShell screen="ob5" step={3} title="공부 성향 맞춤 솔루션" crackySrc={crackySrc} subcopy={<>저장된 성적과 서버 분석을 바탕으로<br />확인 가능한 전략만 보여드려요.</>} bubble="아직 계산되지 않은 결과는 숫자로 꾸미지 않고, 분석에 필요한 다음 단계를 안내해요." cta={<><button type="button" className="cta-button" data-action="startStandard">Standard 기능 살펴보기</button><button type="button" className="auth-link-btn" data-action="completeOnboarding">홈으로 이동</button></>}>
    ...
    <div className="ob5-after-eta"><div className="card ob-card"><p className="analysis-title">환산점수 변화</p>{hasScore ? <><div className="ob-total-compare"><div><span>현재</span><b>{formatPoint(gaugeCurrent)}점</b></div><i>→</i><div><span>목표</span><b className="target">미확인</b></div></div><Gauge current={gaugeCurrent} currentPct={gaugeCurrentPct} target={targetScore} targetPct={targetPct} passPct={gaugePassPct} safePct={gaugeSafePct} scoreTierClass={scoreTierClass} /><p className="sub"><b>현재 계산 결과 기준 · 목표 성적 미확인</b></p></> : <OnboardingState kind={isLoading ? 'loading' : analysisApiError ? 'error' : 'empty'} title={isLoading ? '환산 분석을 확인 중이에요' : analysisApiError || '표시할 환산 결과가 없어요'} description="분석 화면에서 저장된 성적과 목표 대학의 환산점수를 먼저 계산해주세요." action={!isLoading ? <button type="button" className="btn btn-secondary mini" data-action="goto" data-target="analysis">분석 화면으로 이동</button> : null} />}</div>
