...
function CoachingHero({ submitted = false }) {
  return (
    <section className="coaching-hero">
      <div className="coaching-hero-copy"><span>SKY 선배 1:1 멘토링</span><h3>이번 주 공부, 혼자 고민하지 마세요</h3><p>학습 기록과 고민을 보내면 다음 주 방향을 구체적인 피드백으로 정리해 드려요.</p></div>
      <CoachingMark />
...
function CoachingEmpty({ view = 'sessions', loading = false }) {
  ...
  return <div className={`coaching-empty ${loading ? 'is-loading' : ''}`}><span aria-hidden="true">{loading ? '···' : '✓'}</span><b>{copy[0]}</b><p>{copy[1]}</p></div>;
}
...
function CoachingSheet(ctx) {
  ...
  return <div className="sc-sheet-overlay coach-sheet-overlay" data-action="closeCoachingSheet"><section className="sc-sheet coach-sheet" data-action="noopModal"><div className="sc-sheet-handle" aria-hidden="true" /><header className="coach-sheet-head"><div><span>주간 학습 점검</span><h3>튜터에게 보낼 이번 주 기록</h3></div><button type="button" className="coach-close" data-action="closeCoachingSheet" aria-label="닫기">×</button></header><div className="coach-step-progress"><i style={{ width: `${(coachingStep / 8) * 100}%` }} /><span>{coachingStep} / 8</span></div><div className="coach-sheet-body"><CoachingStepBody {...ctx} /></div><footer className="coach-sheet-footer"><button type="button" className="btn btn-secondary" data-action="coachingPrev" disabled={coachingStep === 1 || coachingSubmitting}>이전</button><button type="button" className="btn btn-primary" data-action="coachingNext" disabled={coachingSubmitting}>{coachingStep === 8 ? (coachingSubmitting ? '제출 중' : '작성 완료 및 제출') : '다음 단계'}</button></footer></section></div>;
}

export function CoachingScreen(ctx) {
  const { coachingView = 'sessions', dimmed = false, tabBarHtml = '', weeklyReports = [], weeklyReportsStatus = 'idle' } = ctx;
  ...
  return <div className="app-shell"><div className="app-frame"><div className={`screen app-screen app-content ${dimmed ? 'modal-lock' : ''}`} data-screen="strategy"><main className="coach-page coaching-screen"><header className="coaching-context"><div><span>학습 코칭</span><h2>선배와 함께 다음 주를 설계해요</h2><p>주간 기록을 점검하고, 바로 실행할 피드백을 받아보세요.</p></div><CoachingMark /></header><CoachingHero submitted={presentation.submitted} /><section className="coaching-history"><div className="coaching-history-head"><div><span>코칭 내역</span><h3>{activeView === 'feedback' ? '받은 피드백' : '이번 주 점검'}</h3></div>{presentation.feedbackReady ? <em>새 피드백</em> : null}</div><CoachingSegment active={activeView} /><div className="coaching-history-list">{rows.length ? rows.map((item) => activeView === 'feedback' ? <FeedbackRow item={item} key={item.weekId} /> : <SessionRow item={item} key={item.weekId} />) : <CoachingEmpty view={activeView} loading={presentation.isLoading} />}</div>{activeView === 'sessions' && !presentation.isLoading ? <button type="button" className="coaching-new-request" data-action="openCoachingSheet"><span>+</span><b>새 학습 점검 작성</b><small>이번 주 기록과 질문 남기기</small></button> : null}</section></main></div><CoachingSheet {...ctx} /><div style={{ display: 'contents' }} dangerouslySetInnerHTML={{ __html: tabBarHtml }} /></div></div>;
}
