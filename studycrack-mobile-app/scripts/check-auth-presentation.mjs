...
const [authSource, authFallbackSource, authHandlers, authCss, signupCss, recoveryCss, onboardingSource, onboardingCss, splashCss] = await Promise.all([
  readFile(new URL('../src/screens/auth/AuthScreens.jsx', import.meta.url), 'utf8'),
  readFile(new URL('../src/screens/auth/renderers.js', import.meta.url), 'utf8'),
...
assert.match(authSource, /renderTermsModal\(openTermsType\)/);
assert.match(authFallbackSource, /renderSignupProgress\(step\)/);
assert.doesNotMatch(authFallbackSource, /소셜 계정으로 시작하기/);
