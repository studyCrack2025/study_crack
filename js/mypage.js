...
function getSimpleAdvice(score, status) {
    // 1. [초고득점] Z-Score +3.0 이상 (137.5점 근처)
    if (score >= 140) {
        return `<strong>👑 최초 합격 / 장학금 유력</strong> 구간입니다.<br>
        사실상 합격할 가능성이 매우 높습니다. 장학금 혜택을 확인하거나, 더 높은 대학을 과감하게 상향 지원해보는 전략이 필요합니다.`;
    }

    // 2. [최초합 안정] Z-Score +2.0 이상 (125점)
    if (score >= 125) {
        return `<strong>매우 안정 (최초합 유력)</strong>입니다.<br>
        심리적 마지노선으로 잡기에 완벽한 카드입니다. 이 대학을 보험으로 두고 상향 지원 전략을 짜세요.`;
    }
    
    // 3. [안정] Z-Score +1.0 이상 (112.5점)
    if (score >= 112) {
        return `<strong>합격 가능성이 높습니다. (안정)</strong><br>
        경쟁률이 이례적으로 폭등하지 않는 한 무난한 합격이 예상됩니다. 안정 지원 카드로 활용하세요.`;
    }
    
    // 4. [적정] Z-Score 0.0 이상 (100점) - 합격 컷 부근
    if (score >= 100) {
        return `<strong>적정 지원 (추합권)</strong>입니다.<br>
        예상 합격선 부근입니다. 최초합보다는 추가 합격 가능성이 높으며, 학과 내 경쟁률 변화를 끝까지 주시해야 합니다.`;
    }
    
    // 5. [소신] Z-Score -1.0 이상 (87.5점)
    if (score >= 87) {
        return `<strong>소신 지원 (문 닫고 입학)</strong> 전략입니다.<br>
        합격선보다 낮지만, 충원율이 높은 학과라면 역전 가능성이 있습니다. 불합격 리스크를 감수해야 합니다.`;
    }

    // 6. [위험/상향] Z-Score -2.0 이상 (75점)
    if (score >= 75) {
        return `<strong>상향 지원 (위험)</strong>입니다.<br>
        점수 차이가 꽤 큽니다. 일반적인 상황에서는 합격이 어렵습니다. 반드시 다른 군에 확실한 안정 카드를 확보하세요.`;
    }
    
    // 7. [초고위험] Z-Score -2.0 미만 (75점 미만)
    return `<strong>지원 불가 / 초고위험</strong> 구간입니다.<br>
    현재 점수로는 합격 확률이 매우 희박합니다. 해당 학과보다는 눈높이를 낮추거나 전형을 변경하는 것을 강력히 권장합니다.`;
}
