...
const CRACKY_SRC = 'data:image/svg+xml;utf8,' + encodeURIComponent(`
<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 120 120">
  <defs><linearGradient id="g" x1="0" y1="0" x2="1" y2="1"><stop offset="0" stop-color="#60a5fa"/><stop offset="1" stop-color="#1d4ed8"/></linearGradient></defs>
  <rect x="8" y="18" rx="16" ry="16" width="104" height="94" fill="url(#g)"/>
  <circle cx="45" cy="60" r="9" fill="#fff"/><circle cx="75" cy="60" r="9" fill="#fff"/>
  <circle cx="45" cy="60" r="4" fill="#0f172a"/><circle cx="75" cy="60" r="4" fill="#0f172a"/>
  <rect x="34" y="78" width="52" height="14" rx="7" fill="#f8fafc"/>
  <path d="M30 20l30-14 30 14-30 10z" fill="#1e293b"/>
</svg>`);
...
function App() {
  ...
  const homeView = () => `<div class="brand-row"><p class="greeting">안녕하세요, 지민님 👋</p><img class="brand-logo-sm" src="./assets/images/studycrack_logo_wo_bg.png" alt="logo"/></div>
    <div class="section"><div class="card"><p class="sub">내 합격 가능성</p><div style="display:flex;justify-content:space-between;align-items:center"><div><p class="metric">68%</p><p class="sub">상위 32%</p></div><div class="ring"></div></div>
    ...
    <div class="section"><div class="quick-grid">${quick('analysis','chart','분석')}${quick('strategy','target','전략')}${quick('planner','calendar','플래너')}${quick('weekly','check','주간점검')}${quick('report','report','프로보고서')}${quick('tutor','chat','SKY튜터')}</div></div>
  ...
  const screens = {
    ...
    on1: layout(appbar('온보딩 1', true) + `<div class="card"><p class="sub">데이터 기반 합격 가능성 분석</p><p class="metric-sm">72%</p><div class="progress"><span style="width:72%"></span></div>${mascotBubble('흔들리지 않는 방향을 찾았어요!')}</div><button class="btn btn-primary" data-action="goto" data-target="on2">다음</button>`, false),
    on2: layout(appbar('온보딩 2', true) + `<div class="card"><p class="sub">점수 상승 전략 역산</p><ul class="list"><li>수학 +12점 → 합격 가능성 +18%</li><li>탐구 +1등급 → 합격 가능성 +9%</li></ul>${mascotBubble('과목 ROI부터 공략하면 빨라요!')}</div><button class="btn btn-primary" data-action="goto" data-target="on3">다음</button>`, false),
    on3: layout(appbar('온보딩 3', true) + `<div class="card"><p class="sub">실행부터 관리까지 함께</p><ul class="list"><li>플래너</li><li>주간 점검</li><li>SKY튜터 피드백</li><li>프로 보고서</li></ul></div><button class="btn btn-primary" data-action="goto" data-target="home">홈으로</button>`, false),
    home: layout(homeView(), true),
    analysis: layout(appbar('분석', false) + `<div class="card"><span class="badge">연세대학교 경영학과</span><p class="metric-sm">68%</p><p class="sub">현재 323점 / 합격 컷 335점 / 부족 -12점</p></div><div class="card"><div class="bar">수학<div class="track"><i style="width:88%"></i></div><span>+15%</span></div><div class="bar">탐구<div class="track"><i style="width:72%;background:#0ea5a2"></i></div><span>+9%</span></div></div>`, true),
    ...
    weekly: layout(appbar('주간 점검', true) + `<div class="card"><p class="sub">플래너 수행률</p><p class="metric-sm">82%</p><div class="progress"><span style="width:82%"></span></div>${mascotBubble('수학 시간을 더 확보해볼까요?','sm')}</div><button class="btn btn-primary" data-action="back">다음 주 계획 세우기</button>`, false),
    report: layout(appbar('프로 보고서', true) + `<div class="card"><span class="badge">Pro 플랜 전용</span><p class="sub">2주에 1회 제공</p><div class="card" style="padding:14px"><div style="display:flex;justify-content:space-between;align-items:center"><div><p class="metric-sm">D-11</p><p class="sub">5월 25일 (토)</p></div><img src="${CRACKY_SRC}" class="cracky-img cracky-md" alt="크랙이"/></div></div></div><div class="card"><ul class="list"><li data-action="goto" data-target="reportDetail">5월 11일 · 종합 분석 리포트</li><li>4월 27일 · 중간 분석 리포트</li></ul></div><button class="btn btn-primary">PDF 다운로드</button>`, false),
    reportDetail: layout(appbar('프로 보고서 상세', true) + `<div class="card"><ul class="list"><li>수학 점수 상승 여지 큼</li><li>목표 대학 거리 -12점</li><li>중기: 탐구 집중 강화</li></ul></div>`, false),
