...
function App() {
  ...
  const screens = {
    ...
    on1: layout(appbar('온보딩 1', true) + `<div class="card"><p class="sub">데이터 기반 합격 가능성 분석</p><p class="metric-sm">72%</p><div class="progress"><span style="width:72%"></span></div></div><button class="btn btn-primary" data-action="goto" data-target="on2">다음</button>`, false),
    on2: layout(appbar('온보딩 2', true) + `<div class="card"><p class="sub">점수 상승 전략 역산</p><ul class="list"><li>수학 +12점 → 합격 가능성 +18%</li><li>탐구 +1등급 → 합격 가능성 +9%</li></ul></div><button class="btn btn-primary" data-action="goto" data-target="on3">다음</button>`, false),
