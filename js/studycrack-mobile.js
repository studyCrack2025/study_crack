...
function App() {
  ...
  const screens = {
    ...
    authSignup: layout(appbar('회원가입', true) + `<div class="signup-page"><div class="signup-form-card">
      <div class="auth-logo-wrap compact"><img src="${STUDYCRACK_LOGO_SRC}" class="auth-logo" alt="StudyCrack Logo" onerror="this.style.display='none'; this.nextElementSibling.style.display='block';" /><span class="auth-logo-fallback">StudyCrack</span></div>
      <p class="signup-title">회원가입</p>
      <div class="signup-section"><p class="section-title">1. 계정 정보</p><div class="section-divider"></div><label class="auth-label">이메일(아이디)</label><div class="input-row"><input class="input" data-field="signupEmail" value="${signupEmail}" placeholder="example@email.com" /><button class="input-btn" data-action="verifySignupEmail">인증번호 받기</button></div><label class="auth-label">비밀번호</label><input class="input" data-field="signupPassword" value="${signupPassword}" type="password" placeholder="영문 대/소문자, 숫자, 특수문자 포함 8자 이상" /><label class="auth-label">비밀번호 확인</label><input class="input" data-field="signupPasswordConfirm" value="${signupPasswordConfirm}" type="password" placeholder="비밀번호 재입력" /></div>
      <div class="signup-section"><p class="section-title">2. 개인 정보</p><div class="section-divider"></div><label class="auth-label">이름(실명)</label><input class="input" data-field="signupName" value="${signupName}" placeholder="이름 입력" /><div class="grid-2"><div class="radio-group"><label class="radio-item"><input type="radio" name="signupGender" data-action="setSignupGender" data-gender="female" ${signupGender==='female'?'checked':''}/>여성</label><label class="radio-item"><input type="radio" name="signupGender" data-action="setSignupGender" data-gender="male" ${signupGender==='male'?'checked':''}/>남성</label></div><input class="input" data-field="signupBirth" value="${signupBirth}" placeholder="생년월일 8자리" /></div><label class="auth-label">전화번호</label><div class="input-row"><input class="input" data-field="signupPhone" value="${signupPhone}" placeholder="- 없이 입력해주세요" /><button class="input-btn" data-action="verifySignupPhone">인증번호 전송</button></div></div>
      <div class="signup-section"><p class="section-title">3. 세부 정보</p><div class="section-divider"></div><label class="auth-label">희망 계열</label><div class="radio-group"><label class="radio-item"><input type="radio" name="track"/>의치한약계열</label><label class="radio-item"><input type="radio" name="track"/>자연/공학계열</label><label class="radio-item"><input type="radio" name="track"/>상경계열</label><label class="radio-item"><input type="radio" name="track"/>어문/사회계열</label><label class="radio-item"><input type="radio" name="track"/>예체능</label><label class="radio-item"><input type="radio" name="track"/>기타</label></div><label class="auth-label">유입 경로</label><div class="radio-group"><label class="radio-item"><input type="radio" name="source"/>인스타그램</label><label class="radio-item"><input type="radio" name="source"/>스레드</label><label class="radio-item"><input type="radio" name="source"/>오르비</label><label class="radio-item"><input type="radio" name="source"/>기타</label></div><label class="auth-label">프로모션 코드(선택)</label><input class="input" placeholder="프로모션 코드 입력" /></div>
      <div class="terms-card"><div class="terms-header"><input type="checkbox" data-action="toggleSignupTermsAll" ${signupTermsAll?'checked':''}/><span>약관 전체 동의</span></div><div class="terms-item"><label><input type="checkbox" data-action="toggleSignupTermsRequired" ${signupTermsRequired?'checked':''}/> [필수] 서비스 이용약관</label><button class="terms-link" data-action="goto" data-target="termsScreen">전문보기</button></div><div class="terms-item"><label><input type="checkbox" data-action="toggleSignupTermsRequired" ${signupTermsRequired?'checked':''}/> [필수] 개인정보 수집 및 이용</label><button class="terms-link" data-action="goto" data-target="termsScreen">전문보기</button></div><div class="terms-item"><label><input type="checkbox" data-action="toggleSignupTermsRequired" ${signupTermsRequired?'checked':''}/> [필수] 만 14세 이상 확인</label><button class="terms-link" data-action="goto" data-target="termsScreen">전문보기</button></div><div class="terms-item"><label><input type="checkbox" data-action="toggleSignupTermsRequired" ${signupTermsRequired?'checked':''}/> [필수] 제3자 제공 동의</label><button class="terms-link" data-action="goto" data-target="termsScreen">전문보기</button></div><div class="terms-item"><label><input type="checkbox"/> [선택] 마케팅 정보 수신 동의</label><button class="terms-link" data-action="goto" data-target="termsScreen">전문보기</button></div></div>
      <button class="signup-submit signup-submit-btn ${signupSubmitEnabled ? 'active' : 'disabled'}" data-action="signupSuccess" ${signupSubmitEnabled ? '' : 'disabled'}>회원가입 완료</button>
